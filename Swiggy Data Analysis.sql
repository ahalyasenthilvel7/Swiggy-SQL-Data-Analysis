...
# 24.Display deliveries where the delivery rating is not available.# is null
select * 
from delivery
where DeliveryRating is null;

# 25.Display deliveries that have received ratings. # is not null
select *
from delivery
...
price + (price * 0.5) as price_with_service_charge
from MenuItems;
...
select avg(FoodRating) as avg_customer_review_rating
from reviews;

# 50.Display the average review rating rounded to two decimal places.# round()
select round(avg(FoodRating),2) as rounded_avg_review_rating
from reviews;
...
select PaymentMethod,count(*) as no_of_transaction
from payments
group by Paymentmethod;
...
select categoryID,avg(price) as avg_menu_price
from menuitems
group by categoryID;
...
# 61.Display customer name, order ID, order date, and total amount.
 /*Tables Used: Orders + Customers (INNER JOIN on CustomerID)*/
...
#Tables Used: Orders + Restaurants (INNER JOIN on RestaurantID)
select O.OrderID,
	   R.RestaurantName,
       O.OrderDate,
       O.TotalAmount
...
#Tables Used: MenuItems + MenuCategories (INNER JOIN on CategoryID)
select M.ItemName,
	   MC.CategoryName,
	   M.Price
from MenuItems as M
...
# Tables Used: Payments + Orders (INNER JOIN on OrderID)
select O.OrderID,
...
# Tables Used: Delivery + DeliveryPartners (INNER JOIN on PartnerID)
select d.OrderID,
	   dp.PartnerName,
	   d.DeliveryStatus
from delivery as d
...
# Tables Used: Customers + Reviews (INNER JOIN on CustomerID)
...
# Tables Used: Restaurants + Reviews (INNER JOIN on RestaurantID)
...
# Tables Used: Restaurants + MenuItems (INNER JOIN on RestaurantID)
select R.RestaurantName,
...
# Tables Used: Customers + Orders (LEFT JOIN on CustomerID)
select C.CustomerID,
...
# Tables Used: Restaurants + MenuItems (LEFT JOIN on RestaurantID)
select R.RestaurantName,
       M.ItemName
...
# Tables Used: Orders + Payments (LEFT JOIN on OrderID)
select
    O.OrderID,
    P.PaymentID,
    P.Amount,
...
# Tables Used: Orders + Delivery (LEFT JOIN on OrderID)
select O.OrderID,
	   O.RestaurantID,
       D.DeliveryID,
       D.DeliveryTime,
...
# Tables Used: Restaurants + Reviews (LEFT JOIN on RestaurantID)
...
left join Reviews as RW
	on RW.RestaurantID = R.RestaurantID;
    
# 74.Display all menu categories along with their menu items.
# Tables Used: MenuCategories + ManuItems (LEFT JOIN on CategoryID)
...
# Tables Used: Payments + Orders (LEFT JOIN on OrderID) 
select P.PaymentID,
	   P.OrderID,
       P.PaymentStatus,
       O.CustomerID,
...
# Tables Used: Reviews + Restaurants (LEFT JOIN on RestaurantID)
select RW.reviewID,
	   RW.OrderID,
       RW.ReviewComment,
       R.RestaurantID,
...
# Tables Used: delivery + deliveryPartners (LEFT JOIN on partnerID) 
select 
	d.orderID,
    d.deliveryID,
    d.deliverystatus,
...
# Tables Used: Customers + Orders + Restaurants + Payments
# (INNER JOIN on CustomerID, RestaurantID, and OrderID)
select concat(C.FirstName,' ',C.LastName) as CustomerName,
	   R.RestaurantName,
       O.TotalAmount,
       P.PaymentStatus
...
# Tables Used: Customers + Orders + Restaurants + Deliveries + DeliveryPartners
# (INNER JOIN on CustomerID, RestaurantID, OrderID, and DeliveryPartnerID)
...
# Tables Used: Customers + Orders + Restaurants + Payments + Reviews
# (INNER JOIN on CustomerID, RestaurantID and OrderID)
select concat(C.FirstName,' ',C.LastName) as CustomerName,
	   R.RestaurantName,
       P.Amount,
...
inner join Payments as P
	on O.OrderID = P.OrderID
inner join Reviews as RW
	on O.OrderID = RW.OrderID;
...
select	R.RestaurantID,
		R.RestaurantName,
		avg(RW.FoodRating) as AverageRating
from Restaurants as R
left join Reviews as RW
    on R.RestaurantID = RW.RestaurantID
...
select DP.PartnerID,
	   DP.PartnerName, 
       count(D.PartnerID) as no_of_deliveries 
...
select MC.CategoryName, 
	   count(MI.ItemName) as No_Of_Menu_items 
from MenuCategories as MC 
inner join MenuItems as MI 
	on MC.CategoryID = MI.CategoryID 
group by CategoryName;

# 90Prepare a consolidated order report containing customer, restaurant, payment, and delivery details. 
select O.OrderID,
       C.FirstName,
       R.RestaurantName,
       O.TotalAmount,
...
select O.OrderID,
       O.OrderDate,
       D.Deliverytime,
       timestampdiff(minute,O.OrderDate,D.deliveryTime) as delivery_duration_minutes
from orders as O
inner join Delivery as D
	on O.OrderID = D.OrderID;
...
select OrderID,
	   OrderDate,
       date_sub(Orderdate,interval 3 day) as Remainder_date
from Orders;

# 110.Add seven days to each order date.
select OrderID,
	   OrderDate,
       date_add(OrderDate,interval 7 day) as OrderDate_Plus_7_Days
from Orders;

# 111.Subtract five days from each order date.
select OrderID,
	   OrderDate,
       date_sub(OrderDate,interval 5 day) as OrderDate_minus_5_Days
from Orders;
...
select OrderDate,
	   date_format(OrderDate, '%M-%Y') as formatted_month_Year
from Orders;
	   
# 115.Display monthly revenue generated from completed payments.
select date_format(Paymentdate, '%M-%Y') as Month_Year,
...
select dayname(OrderDate) as Order_weekday,
	   count(OrderID) as Orders_count
from Orders
...
select avg(timestampdiff(minute,D.DeliveryTime, O.OrderDate )) as Average_Delivery_Time_Minutes
from Orders as O
inner join Delivery as D
    on O.OrderID = D.OrderID;
...
select C.CustomerID,
       ...
       row_number() over(order by sum(O.TotalAmount)desc) as RowNumber
from Customers as C
inner join Orders as O
	on C.customerID = O.CustomerID
group by CustomerID;
...
select rank() over(order by sum(O.TotalAmount)desc) as Restaurant_Rank,
	   R.RestaurantName,
       R.RestaurantID,
...
select dense_rank() over (order by sum(O.TotalAmount) desc) as Customer_Rank,
		C.CustomerID,
...
select PaymentID,
	   PaymentDate,
       Amount,
       avg(Amount) over (order by PaymentDate) as mov_avg_payments
from Payments 
where PaymentStatus = 'success';
use swiggydb;
...
with Customer_wise_Spending as
(
	select concat(C.firstName,' ',C.LastName) as CustomerName,
           sum(O.TotalAmount) as Total_Spending
	from Customers as C
...
( 
   select R.RestaurantID,
		  avg(O.TotalAmount) as Avg_value
   from orders as O
   inner join Restaurants as R
	on R.restaurantID = O.RestaurantID
...
(select O.OrderID,
       ...
       O.TotalAmount
from Orders as O
inner join Avg_order_Value as A
	on A.RestaurantID = O.RestaurantID
where O.TotalAmount > Avg_Value);
...
select OrderID,
	   ...
	   TotalAmount,
       case
			when TotalAmount < 350 then 'low'
            when TotalAmount between 350 and 650 then 'medium'
            else 'high'
...
select R.restaurantID,
	   R.RestaurantName,
       avg(RW.foodrating) as avg_food_rating
from Restaurants as R
inner join Reviews as RW
	on RW.RestaurantID = R.RestaurantID
group by R.RestaurantID,R.RestaurantName
having avg_food_rating > 4.5;
